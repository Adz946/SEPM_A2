...
public class TicketView {
    ...
    public int Menu() {
        ...
        boolean techy = false;
        if (StaffData.Get().GetActiveStaff() instanceof Technician) { techy = true; }

        ArrayList<Ticket> tickets = TicketData.Get().GetStaffTickets();
        if (!tickets.isEmpty()) { 
            ...
            if (input.equals("1")) { 
                Ticket ticket = TicketData.Get().GetTicket(TicketSelect());
                if (ticket != null) { 
                    if (techy) {
            ...
            else if (input.equals("2")) { return 3; }
